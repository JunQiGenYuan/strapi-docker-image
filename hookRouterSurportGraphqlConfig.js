      ...
      if (((cluster.isWorker && strapi.config.reload.workers > 0) || (cluster.isMaster && strapi.config.reload.workers < 1)) || (!strapi.config.reload && cluster.isMaster)) {
        ...
        if (strapi.config.graphql.enabled === true) {
          ...
          strapi.once('waterline:graphql:ready', function () {
            var koaGraphqlOptions = Object.assign({}, strapi.config.graphql);
            delete koaGraphqlOptions.enabled;
            delete koaGraphqlOptions.route;

            strapi.router.all(strapi.config.graphql.route, strapi.middlewares.graphql(Object.assign(koaGraphqlOptions, {
